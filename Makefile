...
PROJECT_REPO ?= github.com/displague/$(PROJECT_NAME)
...
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-equinix_v$(TERRAFORM_PROVIDER_VERSION)
...
xpkg.build.provider-equinix: do.build.images
...
$(TERRAFORM): check-terraform-version
	...
	@curl -fsSL https://releases.hashicorp.com/terraform/$(TERRAFORM_VERSION)/terraform_$(TERRAFORM_VERSION)_$(HOST_PLATFORM).zip -o $(TOOLS_HOST_DIR)/tmp-terraform/terraform.zip
...
#   You can check the basic implementation here: https://github.com/crossplane/uptest/blob/main/internal/equinixs/03-delete.yaml.tmpl.
