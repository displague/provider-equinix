...
import (
	...
	clusterv1beta1 "github.com/displague/provider-equinix/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/displague/provider-equinix/apis/namespaced/v1beta1"
)
...
const (
	...
	// Equinix provider configuration keys
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if v, ok := creds[keyAuthToken]; ok && v != "" {
			ps.Configuration[keyAuthToken] = v
		}
		if v, ok := creds[keyToken]; ok && v != "" {
			ps.Configuration[keyToken] = v
		}
		if v, ok := creds[keyClientID]; ok && v != "" {
			ps.Configuration[keyClientID] = v
		}
		if v, ok := creds[keyClientSecret]; ok && v != "" {
			ps.Configuration[keyClientSecret] = v
		}
		
		// Set optional configuration parameters
		if v, ok := creds[keyEndpoint]; ok && v != "" {
			ps.Configuration[keyEndpoint] = v
		}
		if v, ok := creds[keyRequestTimeout]; ok && v != "" {
			ps.Configuration[keyRequestTimeout] = v
		}
		if v, ok := creds[keyResponseMaxPageSize]; ok && v != "" {
			ps.Configuration[keyResponseMaxPageSize] = v
		}
		if v, ok := creds[keyMaxRetries]; ok && v != "" {
			ps.Configuration[keyMaxRetries] = v
		}
		if v, ok := creds[keyMaxRetryWaitSeconds]; ok && v != "" {
			ps.Configuration[keyMaxRetryWaitSeconds] = v
		}
